...
from ..utils.losses import compute_binary_cross_entropy_loss_with_logits
from ..datasets import BlipDataset, ClipDataset
from ..models import BlipClassifier, ClipClassifier
from ..models.blip_classifier import BlipClassificationHead
...
@dataclass
class BoostTrainArguments:
    train_set: Union[BlipDataset, ClipDataset]
    val_set: Union[BlipDataset, ClipDataset]
    do_eval: bool = True
    num_epochs: int = 10
    ...
    compute_loss: Callable = field(default=compute_binary_cross_entropy_loss_with_logits)
    loss_kwargs: dict = field(default_factory=default_loss_kwargs)
    eval_delay: int = 0
    eval_epochs: int = 1
    metric_name_for_best_model: Optional[str] = "f1_score_micro"
    metric_for_best_model: Optional[Callable] = f1_score_micro
...
class BoostTrainer:
    def __init__(
            self, 
            cls_head: Union[BlipClassificationHead, ClipDiscriminator], 
            train_args: BoostTrainArguments,
            pretrained_model: Union[BlipClassifier, ClipClassifier]):
    ...
    def train(self):
        ...
        logging_metric_name = self.train_args.metric_name_for_best_model.replace("_", " ").capitalize() if self.train_args.metric_name_for_best_model is not None else "Not specified"
        ...
        for epoch in tqdm(range(self.train_args.num_epochs)):
            ...
            for inputs, labels in train_loader:
                ...
                multimodal_features = self.pretrained_model.get_multimodal_features(input_ids, pixel_values, attention_mask)
                logits_from_pretrained = self.pretrained_model.get_logits_from_features(multimodal_features)
                logits_from_cls_head = self.model.get_logits_from_features(multimodal_features)
                
                with autocast(enabled=self.train_args.amp):
                    outputs = logits_from_pretrained + logits_from_cls_head
                ...
                train_metric += self.train_args.metric_for_best_model(outputs, labels) * batch_size if self.train_args.metric_for_best_model is not None else 0
                train_num_samples += batch_size
            
            train_loss = train_loss / train_num_samples
            history["train_loss"].append(train_loss)
            if self.train_args.metric_for_best_model is not None:
                ...
                if (epoch + 1) % self.train_args.eval_epochs == 0:
                    ...
                    with torch.no_grad():
                        ...
                        for inputs, labels in val_loader:
                            ...
                            val_metric += self.train_args.metric_for_best_model(outputs, labels) * batch_size if self.train_args.metric_for_best_model is not None else 0
